...
\]
be the set of best responses of type $s$ to the uniform conjecture. The \emph{anchor} $\beta(s)$ of type $s$ is the uniform distribution on $B^0(s)$. The \emph{level-1 conjecture profile} is $\beta$.
\end{definition}
...
Fix $t$, distinct $b, b'$ and $\varepsilon \in (0,1)$. Under $\gamma^\varepsilon$,
\[
